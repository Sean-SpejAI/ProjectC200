...
interface ReloadFromSorButtonProps {
  claimId: string;
  claimNumber?: string;
  onReloadStarted?: (runId: string) => void;
}

// Admin-only control. Wipes the claim's local state (manual + SOR-sourced
// docs, AI analysis, claim details edits) and re-pulls everything from
// System of Record. Surface a hard confirmation before invoking.
export function ReloadFromSorButton({ claimId, claimNumber, onReloadStarted }: ReloadFromSorButtonProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast({
        title: "Reload from System of Record started",
  ...
  return (
    <>
      ...
      >
        ...
        <span className="text-label-md hidden md:inline">Reload from System of Record</span>
      </Button>

      <AlertDialog open={open} onOpenChange={setOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Reload this claim from System of Record?</AlertDialogTitle>
            <AlertDialogDescription className="space-y-2">
              <span className="block">
                ...
                re-pull everything from System of Record.
              </span>
              <span className="block">
          ...
          <AlertDialogFooter>
            ...
            >
              {submitting ? "Starting..." : "Reload from System of Record"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
