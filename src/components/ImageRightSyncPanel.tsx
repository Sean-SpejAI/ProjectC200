...
export function ImageRightSyncPanel() {
  ...
  const loadRuns = useCallback(async () => {
    // Cast: imageright_sync_runs lands in Database types after the next type regen
  ...
  const startSync = async () => {
    ...
    try {
      ...
      const { error } = await supabase.functions.invoke("imageright-sync", { body });
  ...
  const pullOneClaim = async () => {
    ...
    try {
      ...
      toast.success(`Pulling claim ${trimmedClaimNumber} from ImageRight`, {
  ...
  return (
    <div className="space-y-6 mt-10">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-headline-md text-primary">ImageRight Sync</h2>
          <p className="text-body-md text-on-surface-variant">Pull claims and documents from ImageRight</p>
        </div>
      </div>
      ...
      <Card className="bg-surface-container-lowest border-outline-variant shadow-elevation-1 rounded-2xl p-6">
        ...
        <p className="text-xs text-on-surface-variant mt-3">
          Both modes filter by ImageRight's <code className="font-mono">DateModified</code> — daily diff covers the last 36 hours, one-time covers the chosen window. One-time loads slice into ~30-day chunks and resume across multiple edge function invocations.
        </p>
      </Card>
      ...
      <Card className="bg-surface-container-lowest border-outline-variant shadow-elevation-1 rounded-2xl p-6">
        ...
        <p className="text-body-md text-on-surface-variant mb-4">
          Enter a claim number (e.g. <code className="font-mono">0000372262</code>) to pull that single claim and all of its documents from ImageRight now. If the claim already exists locally, its documents and AI analysis are wiped and re-pulled.
        </p>
        <div className="flex flex-wrap items-end gap-3">
          ...
          >
            {pullingByNumber ? "Pulling..." : "Pull from ImageRight"}
          </Button>
        </div>
      </Card>
      ...
      <AlertDialog open={confirmPullOpen} onOpenChange={(open) => !pullingByNumber && setConfirmPullOpen(open)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Pull claim <span className="font-mono">{trimmedClaimNumber}</span> from ImageRight?</AlertDialogTitle>
            <AlertDialogDescription className="space-y-2">
              <span className="block">
                If this claim already exists locally, all of its current documents and AI analysis will be permanently deleted, then everything will be re-pulled from ImageRight.
              </span>
              <span className="block">
