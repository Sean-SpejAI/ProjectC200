-- ImageRight watchdogs — recover from broken self-reschedule chains and
-- stuck claim_documents rows that the in-function logic can't reach.
--
-- Bugs this addresses (all observed in prod on 2026-06-01):
--
--   1. imageright_sync_runs stuck in status='running' for hours when the
--      EdgeRuntime.waitUntil(rescheduleSelf) chain in imageright-sync dies
...
-- a join against imageright_sync_tasks to find max(updated_at); if no tasks
-- exist for the run yet (e.g. a daily_diff that found 0 claims), we fall
-- back to imageright_sync_runs.started_at.
--
-- For each stale run, posts a {continuation:true, run_id:...} payload back
-- to the imageright-sync function so it can pick up where it left off.
...
AS $$
DECLARE
  v_url     text := current_setting('app.settings.imageright_sync_url', true);
...
REVOKE ALL ON FUNCTION public.imageright_reset_zombie_processing() FROM PUBLIC, anon, authenticated;
