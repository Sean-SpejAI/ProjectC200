-- imageright_sync_tasks: top-N-by-doc-count selection mode
--
...
ALTER TABLE public.imageright_sync_tasks
  ADD COLUMN IF NOT EXISTS total_docs INTEGER;

ALTER TABLE public.imageright_sync_tasks
  DROP CONSTRAINT IF EXISTS imageright_sync_tasks_status_check;

ALTER TABLE public.imageright_sync_tasks
  ADD CONSTRAINT imageright_sync_tasks_status_check
  CHECK (status IN (
    'queued',
    'running',
    'succeeded',
...
CREATE INDEX IF NOT EXISTS idx_imageright_sync_tasks_run_total_docs
  ON public.imageright_sync_tasks (run_id, total_docs DESC NULLS LAST);
