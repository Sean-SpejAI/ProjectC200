-- ImageRight integration — schema, sync state, and daily-diff scheduler.
--
...
ALTER TABLE public.claims
  ...
  ADD COLUMN IF NOT EXISTS imageright_file_id BIGINT,
  ADD COLUMN IF NOT EXISTS imageright_last_modified TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS imageright_synced_at TIMESTAMPTZ,
...
ALTER TABLE public.claims
  ...
  CHECK (source IN ('manual', 'imageright'));
...
CREATE UNIQUE INDEX IF NOT EXISTS claims_imageright_file_id_key
  ON public.claims (imageright_file_id) WHERE imageright_file_id IS NOT NULL;
...
ALTER TABLE public.claim_documents
  ADD COLUMN IF NOT EXISTS source TEXT NOT NULL DEFAULT 'manual',
  ADD COLUMN IF NOT EXISTS imageright_document_id BIGINT,
  ADD COLUMN IF NOT EXISTS imageright_page_ids BIGINT[],
  ADD COLUMN IF NOT EXISTS imageright_last_modified TIMESTAMPTZ;
...
-- Permit ImageRight rows that exist before their binary content lands.
...
CREATE UNIQUE INDEX IF NOT EXISTS claim_documents_imageright_document_id_key
  ON public.claim_documents (imageright_document_id) WHERE imageright_document_id IS NOT NULL;
...
-- =========================================================================
-- 3. imageright_sync_runs — audit log per sync invocation
-- =========================================================================

CREATE TABLE IF NOT EXISTS public.imageright_sync_runs (
  ...
  notes TEXT,
  CONSTRAINT imageright_sync_runs_run_type_check
    CHECK (run_type IN ('one_time', 'daily_diff', 'manual_reload')),
  CONSTRAINT imageright_sync_runs_status_check
    CHECK (status IN ('running', 'completed', 'failed', 'partial'))
);

CREATE INDEX IF NOT EXISTS idx_imageright_sync_runs_started
  ON public.imageright_sync_runs (started_at DESC);

ALTER TABLE public.imageright_sync_runs ENABLE ROW LEVEL SECURITY;

-- Admins only. Edge functions use the service role and bypass RLS.
DROP POLICY IF EXISTS "Admins can view sync runs" ON public.imageright_sync_runs;
CREATE POLICY "Admins can view sync runs" ON public.imageright_sync_runs
  FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));

-- =========================================================================
-- 4. imageright_sync_tasks — per-claim work queue, supports chunk + resume
-- =========================================================================

CREATE TABLE IF NOT EXISTS public.imageright_sync_tasks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  run_id UUID NOT NULL REFERENCES public.imageright_sync_runs(id) ON DELETE CASCADE,
  imageright_file_id BIGINT NOT NULL,
  ...
  CONSTRAINT imageright_sync_tasks_status_check
    CHECK (status IN ('queued', 'running', 'succeeded', 'failed', 'content_pending'))
);

CREATE INDEX IF NOT EXISTS idx_imageright_sync_tasks_run_status
  ON public.imageright_sync_tasks (run_id, status);
CREATE INDEX IF NOT EXISTS idx_imageright_sync_tasks_file
  ON public.imageright_sync_tasks (imageright_file_id);

DROP TRIGGER IF EXISTS update_imageright_sync_tasks_updated_at ON public.imageright_sync_tasks;
CREATE TRIGGER update_imageright_sync_tasks_updated_at
  BEFORE UPDATE ON public.imageright_sync_tasks
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

ALTER TABLE public.imageright_sync_tasks ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Admins can view sync tasks" ON public.imageright_sync_tasks;
CREATE POLICY "Admins can view sync tasks" ON public.imageright_sync_tasks
  FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
...
--     ALTER DATABASE postgres SET app.settings.imageright_sync_url = '...';
  ...
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron')
    ...
    PERFORM cron.schedule(
      ...
      $cron$
        SELECT
          CASE
            WHEN current_setting('app.settings.imageright_sync_url', true) IS NULL
              OR current_setting('app.settings.service_role_key', true) IS NULL
            THEN NULL
            ELSE net.http_post(
              url := current_setting('app.settings.imageright_sync_url'),
