...
--
-- The legacy approach for keeping an `imageright-sync` run alive across many
...
-- imageright_rescue_stuck_runs (pings the sync URL with {continuation,
...
--     (SELECT jobid FROM cron.job WHERE jobname='imageright-sync-keepalive'),
--     schedule := '* * * * *');
--
-- ...then reverted to NEVER after the reload.

CREATE OR REPLACE FUNCTION public.imageright_sync_keepalive()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.imageright_sync_keepalive() FROM PUBLIC, anon, authenticated;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron')
    ...
    PERFORM cron.unschedule(jobid)
      FROM cron.job WHERE jobname = 'imageright-sync-keepalive';

    PERFORM cron.schedule(
      'imageright-sync-keepalive',
      '0 0 31 2 *',  -- Feb 31 = never. Enable manually during a curated reload.
      $cron$SELECT public.imageright_sync_keepalive()$cron$
    );
  END IF;
END $$;
