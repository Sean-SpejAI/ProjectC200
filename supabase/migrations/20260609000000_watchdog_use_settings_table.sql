-- Fix the sor_redispatch_stuck_pending watchdog to read its settings
-- (analyze function URL + service role key) from the public.sor_settings
-- table instead of `current_setting('app.settings.*')` GUCs.
--
-- The GUC-based version (originally in 20260602000000_sor_watchdogs.sql)
...
--
-- public.sor_settings is the table the sor-sync orchestrator
-- already uses for the same values (see 20260602060000_*_sor_settings.sql
-- or similar). The values are: analyze_document_url, service_role_key,
-- sor_sync_url. Reading from this table works in any session, no GUC needed.
--
-- KNOWN: sor_redispatch_stuck_runs (the SIBLING watchdog in the same
-- 20260602000000 migration) AND the integration cron trigger in
-- 20260518021407_sor_integration.sql ALSO use the broken GUC pattern.
...
DECLARE
  ...
  r        record;
BEGIN
  SELECT value INTO v_url FROM public.sor_settings WHERE name = 'analyze_document_url';
  SELECT value INTO v_key FROM public.sor_settings WHERE name = 'service_role_key';
