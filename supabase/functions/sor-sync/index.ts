// Orchestrator for Sor ingestion. Three entry modes:
...
// one. See feedback_sor_sync_gateway.md for the why.
//
...
} from "../_shared/sor-pull-claim.ts";
...
// initial chunk, far more than we'd ever need (a typical client runs ~100 claims/mo).
...
interface SyncRequest {
  ...
  // orchestrator skips claims whose Sor file has < min_docs documents
  ...
  // in sor_sync_runs.notes as JSON so continuations pick them up.
  min_docs?: number;
  claim_limit?: number;
...
async function appendRunError(
  ...
  await supabase.from("sor_sync_runs").update({ errors }).eq("id", runId);
}
...
async function resetStaleTasks(runId: string, supabase: ReturnType<typeof adminClient>): Promise<number> {
  ...
  const { data, error } = await supabase
    ...
    .select("id");
  if (error) {
    console.error("[sor-sync] stale reset failed", error);
    return 0;
  }
...
async function processNextSlice(
  ...
  // Sor test SOAP randomly returns {files:[]} for slices that DO have
  ...
  if (emptyRetries > 0) {
    console.log(`[sor-sync] slice ${slice.from}..${slice.to} empty-retry x${emptyRetries} → ${res.ok ? res.files.length : "err"} files`);
  }
  ...
  if (!res.ok) {
    ...
    await supabase.from("sor_sync_runs").update({ cursor }).eq("id", runId);
    return true;
  }
  ...
  if (files.length >= RECORDS_CAP) {
    ...
    if (halves) {
      ...
      await supabase.from("sor_sync_runs").update({ cursor }).eq("id", runId);
      console.log(`[sor-sync] slice ${slice.from}..${slice.to} hit cap, split into ${halves[0].from}..${halves[0].to} + ${halves[1].from}..${halves[1].to}`);
      return true;
    }
  ...
  for (const f of files) {
    const { data: existing } = await supabase
      ...
      .select("id")
      .eq("run_id", runId)
      .eq("sor_file_id", f.fileId)
      .maybeSingle();
    if (existing) continue;
    const { error } = await supabase
      .from("sor_sync_tasks")
      .insert({ run_id: runId, sor_file_id: f.fileId, status: "queued" });
    if (!error) inserted += 1;
  }
  ...
  console.log(`[sor-sync] slice ${slice.from}..${slice.to} → ${files.length} files (${inserted} new tasks, ${files.length - inserted} dup)`);
  return true;
}
...
async function processMetadataBatch(
  ...
  const { data: batch } = await supabase
    .from("sor_sync_tasks")
    .select("id, sor_file_id, attempts")
    .eq("run_id", runId)
    .eq("status", "queued")
  ...
  async function probeOne(task: { id: string; sor_file_id: number | string; attempts: number }): Promise<void> {
    await supabase
      .from("sor_sync_tasks")
    ...
    let meta;
    try {
      meta = await pullClaimMetadata(Number(task.sor_file_id));
    } catch (err) {
    ...
    if (!meta.ok) {
      ...
      await appendRunError(supabase, runId, {
        ...
        message: `fileId=${task.sor_file_id} status=${meta.error?.status ?? 0} ${meta.error?.message ?? ""}`,
  ...
  async function worker(): Promise<void> {
    while (true) {
      ...
      try {
        ...
        console.error(`[sor-sync] metadata probe worker crash on task=${task.id}`, err);
      }
      processed += 1;
    }
  }
  ...
  if (queue.length > 0) {
    console.log(`[sor-sync] metadata deadline reached, ${processed}/${batch.length} done, ${queue.length} left in batch`);
  }
...
// sor_file_id ASC for determinism), flips them back to 'queued', and
...
async function selectTopN(
  ...
  async function chunkUpdate(ids: string[], patch: Record<string, unknown>) {
    for (let i = 0; i < ids.length; i += CHUNK) {
      ...
      await supabase.from("sor_sync_tasks").update(patch).in("id", slice);
    }
  }
  ...
  console.log(`[sor-sync] selectTopN: kept ${selectedIds.length}, dropped ${droppedIds.length}, out-of-band ${outOfBandIds.length}, top_n=${topN}, band=[${minTotalDocs ?? "*"}, ${maxTotalDocs ?? "*"}]`);
...
async function processTaskBatch(
  ...
  for (const task of batch) {
    if (Date.now() >= deadline) {
      console.log(`[sor-sync] deadline reached mid-batch (${processed}/${batch.length} done)`);
      break;
    }
    if (filters.claim_limit && keptSoFar >= filters.claim_limit) {
      console.log(`[sor-sync] claim_limit ${filters.claim_limit} hit mid-batch (${processed}/${batch.length} done)`);
      break;
    }
    ...
    let result;
    try {
      result = await pullClaim(Number(task.sor_file_id), {
        runId,
...
async function processSweepBatch(runId: string, supabase: ReturnType<typeof adminClient>): Promise<{ processed: number; remaining: boolean }> {
  const { data: pending } = await supabase
    ...
    .select("id, claim_id, sor_document_id, document_type, file_name")
  ...
  for (const doc of pending) {
    ...
    try {
      const res = await fetch(`${supabaseUrl}/functions/v1/fetch-sor-document`, {
        ...
        body: JSON.stringify({
          documentId: doc.id,
          sor_document_id: doc.sor_document_id,
          claimId: doc.claim_id,
          documentType: doc.document_type ?? "sor-import",
          fileName: doc.file_name ?? `sor-doc-${doc.sor_document_id}.pdf`,
        }),
      });
      ...
      if (res.ok && body?.success) {
        ...
        // fails, the sor_redispatch_stuck_pending watchdog re-fires it.
...
async function rescheduleSelf(runId: string) {
  ...
  const serviceKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
  try {
    await fetch(`${supabaseUrl}/functions/v1/sor-sync`, {
      method: "POST",
      headers: { "Authorization": `Bearer ${serviceKey}`, "Content-Type": "application/json" },
      body: JSON.stringify({ continuation: true, run_id: runId }),
    });
  } catch (err) {
    console.error("[sor-sync] self-reschedule failed", err);
  }
}
...
serve(async (req) => {
  ...
  // Scanner short-circuit (see _shared/scanner-guard.ts). sor-sync
  ...
  // Flag-gated (sor_settings.reconcile_enabled), DEFAULT OFF — until it's
  ...
  if (runType === "daily_diff") {
    try {
      const { data: flagRow } = await supabase
        .from("sor_settings").select("value").eq("name", "reconcile_enabled").maybeSingle();
  ...
  if (resetCount > 0) console.log(`[sor-sync] watchdog reset ${resetCount} stale running tasks`);
