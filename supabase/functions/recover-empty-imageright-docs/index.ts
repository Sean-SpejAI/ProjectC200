// One-shot batch recovery for ImageRight docs that ended up with file_url=NULL
...
// fetch-imageright-document. Bounded per-invocation budget so we stay under
...
const handler = async (req: Request): Promise<Response> => {
  ...
  let query = supabase
    ...
    .select("id, claim_id, imageright_document_id, file_name, document_type")
    ...
    return new Response(
      JSON.stringify({
        ...
        sample: targets.slice(0, 5).map((r) => ({
          id: r.id,
          claim_id: r.claim_id,
          imageright_document_id: r.imageright_document_id,
  ...
  for (const row of targets) {
    ...
    const irDocId = Number(row.imageright_document_id);
    if (!Number.isFinite(irDocId) || irDocId <= 0) {
      results.push({ id: row.id, outcome: "skipped", detail: "missing imageright_document_id" });
      errored += 1;
      continue;
    }
    ...
    try {
      const outcome = await recoverOne({
        ...
        irDocId,
        supabase,
        proxyUrl: IMAGERIGHT_PROXY_URL,
        proxyToken: IMAGERIGHT_PROXY_TOKEN,
      });
...
async function recoverOne(args: RecoverOneArgs): Promise<RecoverOneResult> {
  ...
  const url = `${proxyUrl.replace(/\/$/, "")}/imageright/documents/${encodeURIComponent(String(irDocId))}/pdf`;
  ...
  // fetch-imageright-document so behavior is identical.
  let bytesSeen = 0;
  ...
  const resolvedFileName = row.file_name || `ir-doc-${irDocId}.pdf`;
  const storagePath = `imageright/${storageUuid}/${resolvedFileName}`;
  ...
  if (pageIds.length > 0) updatePayload.imageright_page_ids = pageIds;
