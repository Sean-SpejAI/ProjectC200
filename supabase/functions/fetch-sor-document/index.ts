// PDF content fetch for one Sor document. Called by:
//   - sor-pull-claim (per-claim worker, after the row was created
//     in pending_content state)
//   - sor-sync's pending_content sweep (existing row to retry)
//
// Most documents are one merged PDF: the proxy's GET /sor/documents/:id/pdf
// streams `application/pdf` and surfaces the underlying page ids via the
// `X-Sor-Page-Ids` header. We also store the per-page manifest from
// /pages for the document tree + citations.
//
// LARGE documents: Sor faults when asked to render a very large PDF in
...
interface SorPageManifestEntry {
  n: number | null;
  irPageId: number;
  format: string | null;
  rendered: boolean;
...
async function fetchPdfToStorage(
  ...
  const skippedNonImage = Number(upstream.headers.get("x-sor-skipped-non-image") ?? "0");
  if (skippedNonImage > 0) console.log(`fetch-sor-document: ${skippedNonImage} non-image page(s) excluded`);
...
async function fetchManifest(
  ...
  token: string,
  irDocId: number,
): Promise<{ pages: SorPageManifestEntry[]; renderedCount: number } | null> {
  try {
    const res = await fetch(`${proxyBase}/sor/documents/${encodeURIComponent(String(irDocId))}/pages`, {
      method: "GET",
      headers: { "Authorization": `Bearer ${token}`, "Accept": "application/json" },
    });
    if (!res.ok) return null;
    const m = await res.json() as { pages?: SorPageManifestEntry[]; renderedPageCount?: number };
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!SOR_PROXY_URL || !SOR_PROXY_TOKEN) throw new Error("Sor proxy is not configured");
    ...
    const { documentId, sor_document_id, claimId, documentType, fileName } = body;
    if (!documentId || !sor_document_id || !claimId) {
      return json({ error: "missing_required_fields: documentId, sor_document_id, claimId" }, 400);
    }

    const irDocId = Number(sor_document_id);
    const proxyBase = SOR_PROXY_URL.replace(/\/$/, "");
    const pdfUrl = `${proxyBase}/sor/documents/${encodeURIComponent(String(irDocId))}/pdf`;
    const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);
    const resolvedFileName = fileName || `sor-doc-${irDocId}.pdf`;
    ...
    const manifest = await fetchManifest(proxyBase, SOR_PROXY_TOKEN, irDocId);
    ...
    if (!manifest || renderedCount <= CHUNK_PAGES) {
      const storagePath = `sor/${crypto.randomUUID()}/${sanitizeStorageName(resolvedFileName)}`;
      const r = await fetchPdfToStorage(supabase, pdfUrl, SOR_PROXY_TOKEN, storagePath);
      if (!r.ok) {
      ...
      if (r.pageIds.length > 0) updatePayload.sor_page_ids = r.pageIds;
      if (manifest && manifest.pages.length > 0) updatePayload.sor_pages = manifest.pages;
    ...
    const { data: head } = await supabase
      ...
      .select("claim_id, sor_processing_tier, document_type")
      .eq("id", documentId)
      .maybeSingle();
    const claimIdResolved = (head?.claim_id as string | null) ?? claimId;
    const tier = (head?.sor_processing_tier as string | null) ?? null;
    const headDocType = documentType || (head?.document_type as string | null) || "sor-import";
    ...
    await supabase.from("claim_documents").update({ sor_pages: manifest.pages }).eq("id", documentId);
    ...
    for (let i = 1; i <= partsTotal; i++) {
      ...
      const partPath = `sor/${crypto.randomUUID()}/${sanitizeStorageName(displayName)}`;
      ...
      if (!r.ok) {
        console.log("fetch-sor-document multipart: range fetch failed", { documentId, i, from, to, kind: r.kind, message: r.message });
      ...
      const { data: inserted, error: insErr } = await supabase
        ...
        .insert({
          ...
          processing_status: "pending",
          sor_pages: sliceManifest,
          sor_processing_tier: tier,
          claim_details: {
      ...
      if (insErr) {
        ...
        if ((insErr as { code?: string }).code === "23505") {
          ...
          console.log("fetch-sor-document multipart: part insert failed", { documentId, i, message: insErr.message });
        }
        continue;
      }
      createdThisRun += 1;
    ...
    console.error("fetch-sor-document error:", error);
