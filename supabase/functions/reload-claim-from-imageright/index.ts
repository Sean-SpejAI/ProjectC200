// Admin-triggered "pull from ImageRight" for a single claim. Two entry shapes:
//
//   { claimId: "<uuid>" }           — refresh an existing imageright-sourced
//                                     claim by its Supabase row id.
//   { claimNumber: "0000372262" }   — pull a claim by its ImageRight claim
...
import { pullClaim, resolveFileIdByClaimNumber } from "../_shared/imageright-pull-claim.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    // Resolve to an ImageRight fileId — either from an existing claim row
    // or from a claim-number lookup against the SOAP proxy.
    let imageRightFileId: number;
    let existingClaimId: string | null = null;

    if (claimId) {
      const { data: claim, error: claimErr } = await admin
        .from("claims")
        .select("id, source, imageright_file_id")
        .eq("id", claimId)
        .single();
      ...
      imageRightFileId = Number(claim.imageright_file_id);
      ...
      try {
        ...
        return jsonResponse(502, {
          error: "imageright_lookup_failed",
          message: err instanceof Error ? err.message : String(err),
        });
      }
      if (resolved == null) {
        return jsonResponse(404, { error: "claim_number_not_found_in_imageright" });
      }
      imageRightFileId = resolved;
      // The claim row may or may not exist locally; pullClaim handles both.
      const { data: existing } = await admin
        .from("claims")
        .select("id")
        .eq("imageright_file_id", imageRightFileId)
        .maybeSingle();
    ...
    const { data: run, error: runErr } = await admin
      ...
      .insert({
        ...
        status: "running",
        notes: claimNumber
          ? `claim_number=${claimNumber} file_id=${imageRightFileId}`
          : `claim_id=${existingClaimId} file_id=${imageRightFileId}`,
      })
      .select("id")
      .single();
    if (runErr || !run) throw new Error(`run_insert_failed: ${runErr?.message}`);

    await admin.from("imageright_sync_tasks").insert({
      run_id: run.id,
      imageright_file_id: imageRightFileId,
    ...
    EdgeRuntime.waitUntil((async () => {
      try {
        const result = await pullClaim(imageRightFileId, {
          runId: run.id,
          isReload: true,
        ...
        console.error("[reload-claim-from-imageright] background pull failed", err);
        await admin
          .from("imageright_sync_runs")
          .update({
            status: "failed",
    ...
    console.error("reload-claim-from-imageright error:", err);
