// HTTP client for a System of Record (SOR) connector.
//
// A "connector" is a small per-client adapter that fronts whatever document
// system of record a client runs (e.g. an ECM / claims platform). It exposes
// the JSON + `application/pdf` surface documented in
// docs/SystemOfRecord-Connector-Contract.md; this module never sees the
// upstream protocol. Point it at a connector via SOR_PROXY_URL / SOR_PROXY_TOKEN.
// When those are unset the SOR ingestion pipeline is dormant (see clientFromEnv).
//
...
export interface SorClientConfig {
  proxyUrl: string;
  proxyToken: string;
}
...
export interface SorFileSummary {
  fileId: number;
...
export interface SorFileTreeFile {
  fileId: number;
  claimNumber: string | null;
  fileNumber2: string | null;
  fileNumber3: string | null;
...
export interface SorTreeDocument {
  docId: number;
  ...
  // Short Sor type code (ObjType.Name, e.g. "BIDO") for exact-match filters.
...
export interface SorFileTree {
  file: SorFileTreeFile;
  attributes: Record<string, string | null>;
  documents: SorTreeDocument[];
}

export interface SorDocumentPdf {
  pdfBytes: Uint8Array;
  pageIds: number[];
  contentType: string;
}
...
export function createSorClient(config: SorClientConfig) {
  const { proxyUrl, proxyToken } = config;
  if (!proxyUrl) throw new Error("sor_client: proxyUrl is required");
  if (!proxyToken) throw new Error("sor_client: proxyToken is required");
  ...
  return {
    ...
    async healthUpstream(): Promise<FetchResult> {
      return call("/sor/health-upstream", { method: "GET", retries: 1 });
    },
     ...
     */
    async searchFiles(filter: SearchFilesFilter): Promise<{ ok: boolean; files: SorFileSummary[]; error?: FetchResult }> {
      const res = await call("/sor/files/search", { method: "POST", body: filter });
      if (!res.ok) return { ok: false, files: [], error: res };
      const parsed = res.json<{ files?: SorFileSummary[] }>();
     ...
     */
    async getFileTree(fileId: number | string): Promise<{ ok: boolean; tree?: SorFileTree; error?: FetchResult }> {
      const res = await call(`/sor/files/${encodeURIComponent(String(fileId))}`);
      if (!res.ok) return { ok: false, error: res };
      const tree = res.json<SorFileTree>();
      return { ok: true, tree };
    },
     ...
     * `X-Sor-Page-Ids` (CSV).
     */
    async getDocumentPdf(docId: number | string): Promise<{ ok: boolean; pdf?: SorDocumentPdf; error?: FetchResult }> {
      const res = await call(`/sor/documents/${encodeURIComponent(String(docId))}/pdf`, {
        timeoutMs: DEFAULT_CONTENT_TIMEOUT_MS,
        classify4xxAsTerminal: true,
      });
      if (!res.ok) return { ok: false, error: res };
      const pageIdsHeader = res.headers.get("x-sor-page-ids") ?? "";
    ...
    async getFileInventory(
      ...
      const res = await call(
        `/sor/files/${encodeURIComponent(String(fileId))}/inventory?includeDeleted=${includeDeleted ? "true" : "false"}`,
     ...
     * the claim's document data is actually present in the connected Sor
    ...
    async probeClaim(
      ...
      const res = await call(
        `/sor/claims/${encodeURIComponent(claimNumber)}/probe`,
        { timeoutMs: 120_000, retries: 1 },
      );
      if (!res.ok) return { ok: false, error: res };
    ...
    > {
      const res = await call(`/sor/connections`, { retries: 1 });
      if (!res.ok) return { ok: false, error: res };
...
export type SorClient = ReturnType<typeof createSorClient>;

export function clientFromEnv(): SorClient {
  const proxyUrl = Deno.env.get("SOR_PROXY_URL");
  const proxyToken = Deno.env.get("SOR_PROXY_TOKEN");
  if (!proxyUrl || !proxyToken) {
    throw new Error("SOR_PROXY_URL and SOR_PROXY_TOKEN must be configured");
  }
  return createSorClient({ proxyUrl, proxyToken });
}
