// Per-claim worker: fetches one Sor file (claim) + its documents,
...
//     application/pdf round-trip by the fetch-sor-document worker.
//
// Reused by:
//   - sor-sync (bulk + daily-diff orchestrator)
//   - reload-claim-from-sor (admin-triggered single-claim refresh)
//
// No HTTP entry point — this is a library module.

import { createClient, SupabaseClient } from "https://esm.sh/@supabase/supabase-js@2";
import { dispatchForAnalysis } from "./dispatch-analysis.ts";
import { clientFromEnv, type SorFileTree, type SorTreeDocument } from "./sor-client.ts";
import { tierForType } from "./sor-doc-tier.ts";
...
// Re-export so the orchestrator (sor-sync) can import these from here.
...
export interface PullResult {
  status:
    ...
    // Reconcile aborted: the fresh Sor tree came back empty while we hold
  ...
  // Documents soft-removed because they vanished from Sor.
  docs_removed?: number;
  // Documents present in Sor that were skipped by a folderFilter (if any).
  docs_filtered_out: number;
  // Empty/cut documents (Sor page count 0) skipped at pull — no row,
...
export interface PullOpts {
  ...
  // Curated-load gate: if the Sor file has fewer than `minDocs`
  ...
  // fresh Sor tree against our stored docs and choose incremental (fetch
  ...
  // MORE destructive than this (e.g. Sor changed again so 'incremental'
...
// Pass a TIMESTAMPTZ-friendly value through, nulling Sor's DateTime.MinValue
...
function buildFolderPath(doc: SorTreeDocument): Array<{ id: number | null; name: string }> | null {
...
function passesFolderFilter(doc: SorTreeDocument, filter?: FolderFilter): boolean {
...
// Load reconcile thresholds from sor_settings (name/value rows), falling
...
// and Sor's DateTime.MinValue sentinel (→ null).
...
// Diff the fresh Sor tree against the claim's stored ACTIVE rows.
async function computeTreeDiff(
  supabase: SupabaseClient,
  claimId: string,
  freshDocs: SorTreeDocument[],
): Promise<TreeDiff> {
  const { data: rows } = await supabase
    .from("claim_documents")
    .select("id, sor_document_id, sor_last_modified, sor_page_count, sor_folder_path, claim_details")
    .eq("claim_id", claimId)
    .is("sor_removed_at", null);
  const stored = rows ?? [];

  const storedHeads = stored.filter((r) => r.sor_document_id != null);
  const storedById = new Map<number, (typeof storedHeads)[number]>(
    storedHeads.map((r) => [Number(r.sor_document_id), r]),
  );
  const freshById = new Map<number, SorTreeDocument>(freshDocs.map((d) => [Number(d.docId), d]));

  const added: number[] = [];
  const modified: number[] = [];
  let folderChanged = false;
  const storedFolderSet = new Set(
    storedHeads.map((r) => folderKey(r.sor_folder_path as Array<{ name?: string | null }> | null)),
  );
  ...
  for (const [id, doc] of freshById) {
    ...
    const samePages = doc.pageCount == null || head.sor_page_count === doc.pageCount;
    if (!tsEqual(head.sor_last_modified, doc.dateLastModified) || !samePages) {
      modified.push(id);
    }
    if (fk !== folderKey(head.sor_folder_path as Array<{ name?: string | null }> | null)) {
...
// Soft-remove the given rows (mark sor_removed_at; optionally purge the
...
async function softRemoveDocs(
  ...
  const { error } = await supabase
    .from("claim_documents")
    .update({ sor_removed_at: new Date().toISOString() })
    .in("id", rowIds);
  if (error) {
...
export async function pullClaim(fileId: number, opts: PullOpts): Promise<PullResult> {
  ...
  // Sor's File-level last-modified comes back as DateTime.MinValue,
  ...
  const claimPayload = {
    source: "sor" as const,
    sor_file_id: file.fileId,
    sor_last_modified: claimLastModified,
    sor_synced_at: new Date().toISOString(),
  ...
  // 2. Upsert claim (keyed on sor_file_id) -----------------------------
  const { data: existingClaim } = await supabase
    .from("claims")
    .select("id, synthesis_human_edited_at")
    .eq("sor_file_id", file.fileId)
    .maybeSingle();
  ...
  if (opts.reconcile && existingClaim?.id) {
    ...
    if (mode === "noop") {
      ...
      // scheduled noop (e.g. the held change reverted in Sor) must keep
    ...
    if (mustHold) {
      const nameOf = (docId: number): string => {
        ...
        return d ? (d.description?.trim() || d.documentType?.trim() || `sor-doc-${docId}`) : `sor-doc-${docId}`;
      };
  ...
  for (const doc of documents) {
    ...
    // Skip empty/cut shells (Sor reports pageCount 0). They carry no
...
async function syncOneDocument(
  ...
  const { data: existingDoc } = await supabase
    .from("claim_documents")
    .select("id, processing_status, sor_last_modified, sor_page_count")
    .eq("sor_document_id", doc.docId)
    .maybeSingle();
  ...
  const unchanged = !!existingDoc &&
    tsEqual(existingDoc.sor_last_modified, doc.dateLastModified) &&
    (doc.pageCount == null || existingDoc.sor_page_count === doc.pageCount);
  ...
  const docRow = {
    claim_id: claimId,
    source: "sor" as const,
    sor_document_id: doc.docId,
    sor_last_modified: doc.dateLastModified,
    file_name: defaultFileName(doc),
    document_type: doc.documentType || "sor-import",
    processing_status: "pending_content" as const,
    processing_error: null as string | null,
    // Preserve the Sor hierarchy so the portal can mirror the folder tree
    // and build layered citations. (Per-page manifest is filled by the content
    // fetcher once the PDF lands.)
    sor_folder_id: doc.parentFolderId ?? null,
    sor_folder_name: doc.folderName ?? null,
    sor_folder_path: buildFolderPath(doc),
    sor_document_type_code: doc.documentTypeCode ?? null,
    sor_document_date: normalizeTimestamp(doc.documentDate),
    sor_page_count: doc.pageCount ?? null,
    sor_processing_tier: tierForType(doc.documentTypeCode),
  };
  ...
  const content = await invokeFetchDocument({
    ...
    sor_document_id: doc.docId,
    claimId,
    documentType: doc.documentType || "sor-import",
...
function defaultFileName(doc: SorTreeDocument, withExt = false): string {
  // Prefer the instance Description; when empty, fall back to the Sor
  // document TYPE (ObjType.Description||Name, e.g. "Report") — which mirrors
  // what the Sor desktop client shows for an unnamed document — and only
  // then to the "sor-doc-{id}" placeholder. (Same precedence as soap.js folderLabel.)
  const base = doc.description?.trim() || doc.documentType?.trim() || `sor-doc-${doc.docId}`;
  // Suffix the Sor document id so the name is UNIQUE within a claim.
...
async function invokeFetchDocument(payload: {
  ...
  try {
    const res = await fetch(`${supabaseUrl}/functions/v1/fetch-sor-document`, {
      method: "POST",
      headers: {
...
/**
 * Cheap, DB-free metadata probe for a single Sor file. Used by the
 ...
 * stores the count on `sor_sync_tasks.total_docs` itself.
 */
...
/**
 * Resolve an Sor claim number (e.g. "0000372262") to its fileId by
